    ...
    run(nodes, allNodes) {
        ...
        for (const node of findByTag(nodes, "img")) {
            ...
            if (hasAlt || hasAriaLabel || hasAriaLabelledby) {
                ...
                checkDetails.set(node, {
                    any: [makeCheck("image-alt", "critical", "Element does not have an alt attribute, and does not have role=\"none\" or role=\"presentation\"")],
                });
            }
        }
...
const inputImageAlt = {
    ...
    run(nodes, allNodes) {
        ...
        for (const node of findByTag(nodes, "input")) {
            if (isHiddenOrAncestorHidden(node))
                continue;
            if ((node.attrs.type || "").toLowerCase() !== "image")
                continue;
            ...
            if (name) {
                ...
                violations.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("input-image-alt", "critical", "Element has no accessible name")],
                });
            }
        }
        return { violations, passes, checkDetails };
    ...
    run(nodes, allNodes) {
        ...
        for (const node of findByTag(nodes, "object")) {
            ...
            if (hasAriaLabel || hasAriaLabelledby || hasTitle || hasTextContent) {
                ...
                violations.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("object-alt", "serious", "Element does not have alt text (aria-label, aria-labelledby, title, or text content)")],
                });
            }
        }
        return { violations, passes, checkDetails };
    ...
    run(nodes, allNodes) {
        ...
        for (const node of nodes) {
            ...
            if (name) {
                ...
                violations.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("role-img-alt", "serious", "Element has no accessible name")],
                });
            }
        }
        return { violations, passes, checkDetails };
    ...
    run(nodes, allNodes) {
        ...
        for (const node of findByTag(nodes, "svg")) {
            ...
            if (hasAriaLabel || hasAriaLabelledby || hasTitleChild) {
                ...
                violations.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("svg-img-alt", "serious", "Element has no accessible name")],
                });
            }
        }
        return { violations, passes, checkDetails };
    ...
    run(nodes, allNodes) {
        ...
        for (const node of findByTag(nodes, "area")) {
            ...
            if (hasAlt || hasAriaLabel || hasAriaLabelledby) {
                ...
                violations.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("area-alt", "critical", "Element does not have alt text")],
                });
            }
        }
        return { violations, passes, checkDetails };
    ...
    run(nodes, allNodes) {
        ...
        for (const node of findByTag(nodes, "img")) {
            ...
            if (node.attrs.ismap !== undefined) {
                violations.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("server-side-image-map", "minor", "Element has ismap attribute — server-side image maps should not be used")],
                });
            }
            else {
                passes.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("server-side-image-map", "minor", "Element does not use a server-side image map")],
                });
            }
        }
        return { violations, passes, checkDetails };
...
//# sourceMappingURL=text-alternatives.js.map