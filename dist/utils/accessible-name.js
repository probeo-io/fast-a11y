 ...
 */
export function getAccessibleName(node, allNodes) {
...
function getNativeName(node, allNodes) {
    const tag = node.tag;
    ...
    if (tag === "input" || tag === "select" || tag === "textarea") {
        ...
        if (id) {
            // Find <label for="id">
            const label = allNodes.find((n) => n.tag === "label" && n.attrs.for === id);
            if (label) {
        ...
        if (tag === "input") {
            ...
            if (type === "submit")
                return node.attrs.value || "Submit";
            if (type === "reset")
                return node.attrs.value || "Reset";
            if (type === "button")
                return node.attrs.value || "";
            if (type === "image")
                return node.attrs.alt || node.attrs.value || "";
        }
 ...
 */
export function getAccessibleText(node, allNodes) {
    const parts = [];
    for (const child of node.childNodes) {
        if (child.type === "text") {
            ...
            if (tag === "img") {
                ...
                const svgTitle = (el.childNodes || []).find((c) => c.type === "tag" && c.tagName?.toLowerCase() === "title");
                if (svgTitle) {
...
function getLabelTextExcludingInput(label, input) {
    const parts = [];
    function walk(node) {
        for (const child of node.childNodes) {
            if (child.type === "text") {
