...
import { isHiddenOrAncestorHidden, findByTag, getRole, isFocusable, } from "../tree.js";
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (!role) {
                ...
                if (disallowed.length > 0) {
                    ...
                    checkDetails.set(node, {
                        all: [makeCheck("aria-allowed-attr", "critical", "ARIA attribute(s) not allowed: " + disallowed.map(([a]) => a).join(", "))],
                    });
                }
                else {
                    passes.push(node);
            ...
            if (disallowed.length > 0) {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-allowed-attr", "critical", "ARIA attribute(s) not allowed for role \"" + role + "\": " +
                ...
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-allowed-attr", "critical", "ARIA attributes are allowed for role \"" + role + "\"")],
                });
            }
        }
    ...
    run(nodes) {
        ...
        for (const node of findByTag(nodes, "body")) {
            if (node.attrs["aria-hidden"] === "true") {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-hidden-body", "critical", "aria-hidden=\"true\" must not be present on the document body")],
                });
            }
            else {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-hidden-body", "critical", "Document body does not have aria-hidden")],
                });
            }
        }
        return { violations, passes, checkDetails };
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (selfFocusable || descendantFocusable) {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-hidden-focus", "serious", selfFocusable
                ...
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-hidden-focus", "serious", "Element with aria-hidden=\"true\" has no focusable content")],
                });
            }
        }
        return { violations, passes, checkDetails };
...
const ariaRequiredAttr = {
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (node.tag === "input" && (role === "checkbox" || role === "radio" || role === "switch"))
                continue;
            if (node.tag === "select" && role === "combobox")
                continue;
            if (node.tag === "option")
                continue;
            // headings have implicit level from h1-h6
            if (/^h[1-6]$/.test(node.tag) && role === "heading")
                continue;
            const missing = required.filter((attr) => node.attrs[attr] === undefined);
            if (missing.length > 0) {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-required-attr", "critical", "Required ARIA attribute(s) missing for role \"" + role + "\": " + missing.join(", "))],
                });
            }
            else {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-required-attr", "critical", "All required ARIA attributes are present")],
                });
            }
        }
        return { violations, passes, checkDetails };
...
const ariaRequiredChildren = {
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (!requiredChildren || requiredChildren.length === 0)
                continue;
            ...
            if (node.attrs["aria-owns"]) {
                ...
                checkDetails.set(node, {
                    all: [makeCheck("aria-required-children", "critical", "Element uses aria-owns — cannot fully verify required children")],
                });
                continue;
            }
            if (hasRequiredChild) {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-required-children", "critical", "Element has required child role(s)")],
                });
            }
            else {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-required-children", "critical", "Required child role(s) missing for role \"" + role + "\": " +
...
function hasDescendantWithRole(node, roles) {
    for (const child of node.children) {
        const childRole = getEffectiveRole(child);
        if (childRole && roles.includes(childRole))
            return true;
        if (hasDescendantWithRole(child, roles))
            return true;
    }
    return false;
}
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (found) {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-required-parent", "critical", "Element has the required parent role")],
                });
            }
            else {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-required-parent", "critical", "Required parent role missing for role \"" + role + "\": " +
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (invalidRoles.length > 0) {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-roles", "critical", "Invalid ARIA role(s): " + invalidRoles.join(", "))],
                });
            }
            else {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-roles", "critical", "ARIA role is valid")],
                });
            }
        }
        return { violations, passes, checkDetails };
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (invalid.length > 0) {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-valid-attr", "critical", "Invalid ARIA attribute(s): " + invalid.map(([a]) => a).join(", "))],
                });
            }
            else {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-valid-attr", "critical", "All ARIA attributes are valid")],
                });
            }
        }
        return { violations, passes, checkDetails };
        ...
        for (const node of nodes) {
            ...
            for (const [attr, value] of ariaAttrs) {
                if (!VALID_ARIA_ATTRS.has(attr))
                    continue; // aria-valid-attr handles unknown attrs
                // Boolean attributes
            ...
            if (invalidAttrs.length > 0) {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-valid-attr-value", "critical", "Invalid ARIA attribute value(s): " + invalidAttrs.join(", "))],
                });
            }
            else {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-valid-attr-value", "critical", "All ARIA attribute values are valid")],
                });
            }
        }
        return { violations, passes, checkDetails };
...
const ariaRoledescription = {
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            if (isHiddenOrAncestorHidden(node))
                continue;
            if (node.attrs["aria-roledescription"] === undefined)
                continue;
            const role = getEffectiveRole(node);
            if (role && ROLEDESCRIPTION_SUPPORTED_ROLES.has(role)) {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-roledescription", "serious", "aria-roledescription is used on an element with a valid role")],
                });
            }
            else {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-roledescription", "serious", "aria-roledescription is used on an element without an appropriate role")],
                });
            }
        }
        return { violations, passes, checkDetails };
...
/* ------------------------------------------------------------------ */
function ariaNameCheck(ruleId, impact, targetRoles) {
    return {
        ruleId,
        run(nodes, allNodes) {
            const violations = [];
            ...
            for (const node of nodes) {
                ...
                if (name) {
                    ...
                    violations.push(node);
                    checkDetails.set(node, {
                        any: [makeCheck(ruleId, impact, "Element with role=\"" + role + "\" does not have an accessible name")],
                    });
                }
            }
            return { violations, passes, checkDetails };
...
const ariaInputFieldName = ariaNameCheck("aria-input-field-name", "serious", ["combobox", "listbox", "searchbox", "spinbutton", "textbox"]);
const ariaToggleFieldName = ariaNameCheck("aria-toggle-field-name", "serious", ["checkbox", "menu", "menuitemcheckbox", "menuitemradio", "radio", "radiogroup", "switch"]);
const ariaCommandName = ariaNameCheck("aria-command-name", "serious", ["button", "link", "menuitem"]);
...
const ariaText = {
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            if (isHiddenOrAncestorHidden(node))
                continue;
            if (node.attrs.role !== "text")
                continue;
            if (hasFocusableDescendant(node)) {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-text", "serious", "Element with role=\"text\" has focusable descendants")],
                });
            }
            else {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-text", "serious", "Element with role=\"text\" has no focusable descendants")],
                });
            }
        }
        return { violations, passes, checkDetails };
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (DEPRECATED_ROLES.has(role)) {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-deprecated-role", "minor", "Role \"" + role + "\" is deprecated")],
                });
            }
            else if (VALID_ROLES.has(role)) {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-deprecated-role", "minor", "Role is not deprecated")],
                });
            }
        }
        return { violations, passes, checkDetails };
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (present.length > 0) {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-prohibited-attr", "serious", "Prohibited ARIA attribute(s) for role \"" + role + "\": " + present.join(", "))],
                });
            }
            else {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-prohibited-attr", "serious", "No prohibited ARIA attributes used")],
                });
            }
        }
        return { violations, passes, checkDetails };
...
const ariaBrailleEquivalent = {
    ...
    run(nodes, allNodes) {
        ...
        for (const node of nodes) {
            ...
            if (!hasBrailleLabel && !hasBrailleRoledescription)
                continue;
            const issues = [];
            if (hasBrailleLabel) {
                const name = getAccessibleName(node, allNodes);
            ...
            if (issues.length > 0) {
                ...
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-braille-equivalent", "serious", "Braille attributes have non-braille equivalents")],
                });
            }
        }
        return { violations, passes, checkDetails };
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (!role)
                continue;
            if (!VALID_ROLES.has(role))
                continue; // aria-roles handles invalid roles
            if (NO_ROLE_ELEMENTS.has(node.tag)) {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-allowed-role", "minor", "Element <" + node.tag + "> cannot have a role attribute")],
                });
                continue;
            }
            ...
            if (Array.isArray(allowed) && (allowed.includes(role) || allowed.includes("*"))) {
                ...
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("aria-allowed-role", "minor", "Role \"" + role + "\" is not allowed on <" + node.tag + ">")],
                });
            }
        }
        return { violations, passes, checkDetails };
        ...
        for (const node of nodes) {
            ...
            if (issues.length > 0) {
                ...
                if (ariaAttrs.length > 0) {
                    passes.push(node);
                    checkDetails.set(node, {
                        all: [makeCheck("aria-conditional-attr", "serious", "ARIA attributes are used correctly for the role")],
                    });
                }
            }
        }
...
/* ------------------------------------------------------------------ */
const presentationRoleConflict = {
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            ...
            const hasGlobalAria = getAriaAttrs(node).some(([attr]) => GLOBAL_ARIA_ATTRS.has(attr) && attr !== "aria-hidden");
            if (hasTabindex || hasGlobalAria) {
                ...
                if (hasGlobalAria)
                    reasons.push("has global ARIA attributes");
                checkDetails.set(node, {
                    all: [makeCheck("presentation-role-conflict", "minor", "Element with role=\"" + role + "\" " + reasons.join(" and "))],
                });
            }
            else {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("presentation-role-conflict", "minor", "Element with role=\"" + role + "\" correctly has no global ARIA or tabindex")],
                });
            }
        }
        return { violations, passes, checkDetails };
