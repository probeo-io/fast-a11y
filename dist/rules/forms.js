...
function isLabelableInput(node) {
    if (node.tag === "select" || node.tag === "textarea")
        return true;
    if (node.tag === "input") {
    ...
    run(nodes, allNodes) {
        ...
        for (const node of nodes) {
            ...
            if (name) {
                ...
                checkDetails.set(node, {
                    any: [makeCheck("label", "critical", "Form element does not have an accessible name")],
                });
            }
        }
    ...
    run(nodes, allNodes) {
        ...
        for (const node of findByTag(nodes, "select")) {
            ...
            if (name) {
                ...
                violations.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("select-name", "critical", "Select element does not have an accessible name")],
                });
            }
        }
        return { violations, passes, checkDetails };
    ...
    run(nodes, allNodes) {
        ...
        for (const node of findByTag(nodes, "input")) {
            ...
            if (name) {
                ...
                violations.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("input-button-name", "critical", "Element does not have discernible text")],
                });
            }
        }
        return { violations, passes, checkDetails };
    ...
    run(nodes, allNodes) {
        ...
        for (const node of nodes) {
            ...
            if (name) {
                ...
                violations.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("button-name", "critical", "Element does not have an accessible name")],
                });
            }
        }
        return { violations, passes, checkDetails };
    ...
    run(nodes, allNodes) {
        ...
        for (const node of nodes) {
            ...
            if (count > 1) {
                violations.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("form-field-multiple-labels", "moderate", "Element has " + count + " label elements associated via for attribute")],
                });
            }
            else if (count === 1) {
                passes.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("form-field-multiple-labels", "moderate", "Element has a single label")],
                });
            }
        }
        return { violations, passes, checkDetails };
...
const autocompleteValid = {
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (node.tag === "input") {
                const type = (node.attrs.type || "text").toLowerCase();
                if (!AUTOCOMPLETE_INPUT_TYPES.has(type) && type !== "hidden")
                    continue;
            }
            ...
            if (!value) {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("autocomplete-valid", "serious", "Element has an empty autocomplete attribute")],
                });
                continue;
            }
            ...
            if (valid) {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("autocomplete-valid", "serious", "Element has a valid autocomplete value")],
                });
            }
            else {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("autocomplete-valid", "serious", "Element has an invalid autocomplete value: " + autocomplete)],
                });
            }
        }
        return { violations, passes, checkDetails };
    ...
    run(nodes, allNodes) {
        ...
        for (const node of nodes) {
            ...
            if (hasLabel || hasAriaLabel || hasAriaLabelledby) {
                passes.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("label-title-only", "serious", "Element has a proper label mechanism")],
                });
            }
            else if (hasTitle) {
                violations.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("label-title-only", "serious", "Element is labeled only by the title attribute")],
                });
            }
