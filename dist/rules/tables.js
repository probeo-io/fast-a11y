...
function findAncestorTable(node) {
        ...
        for (const node of findByTag(nodes, "td")) {
            ...
            if (invalidIds.length > 0 || selfRef) {
                ...
                if (invalidIds.length > 0)
                    reasons.push("invalid ID(s): " + invalidIds.join(", "));
                if (selfRef)
                    reasons.push("references itself");
                checkDetails.set(node, {
                    all: [makeCheck("td-headers-attr", "serious", "headers attribute has " + reasons.join("; "))],
                });
            }
            else {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("td-headers-attr", "serious", "All headers IDs are valid references in the same table")],
                });
            }
        }
...
const thHasDataCells = {
    ...
    run(nodes) {
        ...
        for (const table of tables) {
            if (isHiddenOrAncestorHidden(table))
                continue;
            ...
            if (tdCells.length === 0 && thCells.length > 0) {
                ...
                for (const th of thCells) {
                    violations.push(th);
                    checkDetails.set(th, {
                        all: [makeCheck("th-has-data-cells", "serious", "Table header has no associated data cells")],
                    });
                }
                continue;
            }
            ...
            for (const th of thCells) {
                ...
                if (hasAssociatedData) {
                    passes.push(th);
                    checkDetails.set(th, {
                        all: [makeCheck("th-has-data-cells", "serious", "Table header has associated data cells")],
                    });
                }
                else {
                    violations.push(th);
                    ...
                    for (const cell of row) {
                        if (cell.tag === "td" && getNodeText(cell)) {
                            ...
                            checkDetails.set(cell, {
                                all: [makeCheck("td-has-header", "serious", "Data cell in a large table has no associated header")],
                            });
                        }
                    }
                }
            ...
            for (let ri = 0; ri < rows.length; ri++) {
                ...
                for (let ci = 0; ci < row.length; ci++) {
                    ...
                    if (hasHeader) {
                        passes.push(cell);
                        checkDetails.set(cell, {
                            all: [makeCheck("td-has-header", "serious", "Data cell has an associated table header")],
                        });
                    }
                    else {
                        violations.push(cell);
        ...
        for (const table of findByTag(nodes, "table")) {
            ...
            if (summary === caption) {
                ...
                checkDetails.set(table, {
                    all: [makeCheck("table-duplicate-name", "minor", "Table caption and summary have identical text")],
                });
            }
            else {
                passes.push(table);
                checkDetails.set(table, {
                    all: [makeCheck("table-duplicate-name", "minor", "Table caption and summary are different")],
                });
            }
        }
        return { violations, passes, checkDetails };
        ...
        for (const table of findByTag(nodes, "table")) {
            ...
            if (firstRow.length === 1) {
                ...
                if (colspan >= maxCols && maxCols > 1 && getNodeText(cell)) {
                    violations.push(table);
                    checkDetails.set(table, {
                        all: [makeCheck("table-fake-caption", "serious", "First row contains a single cell spanning all columns — use <caption> instead")],
                    });
                    continue;
                }
            }
            passes.push(table);
            checkDetails.set(table, {
                all: [makeCheck("table-fake-caption", "serious", "Table does not use a fake caption")],
            });
        }
        return { violations, passes, checkDetails };
    },
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (node.tag !== "th") {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("scope-attr-valid", "moderate", "scope attribute is only valid on <th> elements")],
                });
                continue;
            }
            if (VALID_SCOPES.has(scope)) {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("scope-attr-valid", "moderate", "scope attribute has a valid value: \"" + scope + "\"")],
                });
            }
            else {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("scope-attr-valid", "moderate", "scope attribute has an invalid value: \"" + scope + "\"")],
                });
            }
        }
        return { violations, passes, checkDetails };
