...
const linkName = {
    ...
    run(nodes, allNodes) {
        ...
        for (const node of nodes) {
            ...
            const isLink = (node.tag === "a" && node.attrs.href !== undefined) ||
            ...
            if (name) {
                ...
                checkDetails.set(node, {
                    any: [makeCheck("link-name", "serious", "Element does not have discernible text")],
                });
            }
        }
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (title && title.trim()) {
                passes.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("frame-title", "serious", "Element has a title attribute: \"" + title.trim() + "\"")],
                });
            }
            else {
                ...
                if (hasAriaLabel || hasAriaLabelledby) {
                    passes.push(node);
                    checkDetails.set(node, {
                        any: [makeCheck("frame-title", "serious", "Element has an accessible name via ARIA")],
                    });
                }
                else {
                    violations.push(node);
                    checkDetails.set(node, {
                        any: [makeCheck("frame-title", "serious", "Element does not have an accessible name")],
                    });
                }
            }
        }
...
/*  frame-title-unique                                                 */
/* ------------------------------------------------------------------ */
const frameTitleUnique = {
    ruleId: "frame-title-unique",
    run(nodes) {
        const violations = [];
        const passes = [];
        const checkDetails = new Map();
        const frames = nodes.filter((n) => (n.tag === "iframe" || n.tag === "frame") && !isHiddenOrAncestorHidden(n));
        // Group by title
            ...
            if (frameList.length > 1) {
                ...
                if (srcs.size > 1) {
                    ...
                    for (const node of frameList) {
                        violations.push(node);
                        checkDetails.set(node, {
                            any: [makeCheck("frame-title-unique", "serious", "Multiple frames have the same title \"" + title + "\" but different content")],
                        });
                    }
                }
                else {
                    for (const node of frameList) {
                        passes.push(node);
                        checkDetails.set(node, {
                            any: [makeCheck("frame-title-unique", "serious", "Frames with same title have same source")],
                        });
                    }
                }
            }
...
const bypass = {
    ...
    run(nodes, allNodes) {
        ...
        if (hasSkipLink || hasLandmarks || hasHeadings) {
            ...
            const methods = [];
            if (hasSkipLink)
                methods.push("skip link");
            if (hasLandmarks)
                methods.push("landmarks");
            if (hasHeadings)
                methods.push("headings");
            checkDetails.set(target, {
                any: [makeCheck("bypass", "serious", "Page has mechanism to bypass repeated blocks: " + methods.join(", "))],
            });
        }
        else {
            violations.push(target);
            checkDetails.set(target, {
                any: [makeCheck("bypass", "serious", "Page does not have a mechanism to bypass repeated blocks")],
            });
        }
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (value > 0) {
                violations.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("tabindex", "serious", "Element has a tabindex greater than 0: tabindex=\"" + node.attrs.tabindex + "\"")],
                });
            }
            else {
                passes.push(node);
                checkDetails.set(node, {
                    all: [makeCheck("tabindex", "serious", "Element has appropriate tabindex value")],
                });
            }
        }
        return { violations, passes, checkDetails };
        ...
        for (const [key, nodeList] of keyMap) {
            if (nodeList.length > 1) {
                for (const node of nodeList) {
                    violations.push(node);
                    checkDetails.set(node, {
                        any: [makeCheck("accesskeys", "serious", "accesskey value \"" + key + "\" is not unique")],
                    });
                }
            }
            else {
        ...
        for (const node of nodes) {
            ...
            if (!isInsideLandmark(node)) {
                ...
                while (p) {
                    ...
                    if (pRole && LANDMARK_ROLES.has(pRole))
                        break;
                    p = p.parent;
                }
                if (isTopLevel && landmarkNodes.has(node)) {
                    ...
                    violations.push(node);
                    checkDetails.set(node, {
                        any: [makeCheck("region", "moderate", "Element is not contained within a landmark region")],
                    });
                }
            }
        }
