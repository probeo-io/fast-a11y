...
function findLandmarksByRole(nodes, role) {
    return nodes.filter((n) => {
        if (isHiddenOrAncestorHidden(n))
            return false;
...
function isInsideLandmark(node) {
    ...
    while (parent) {
        ...
        if (parentRole && LANDMARK_ROLES.has(parentRole))
            return true;
    ...
    run(nodes) {
        ...
        if (mainLandmarks.length >= 1) {
            ...
            checkDetails.set(target, {
                any: [makeCheck("landmark-one-main", "moderate", "Document has a main landmark")],
            });
        }
        else {
            violations.push(target);
            checkDetails.set(target, {
                any: [makeCheck("landmark-one-main", "moderate", "Document does not have a main landmark")],
            });
        }
        return { violations, passes, checkDetails };
    },
...
/* ------------------------------------------------------------------ */
function noDuplicateLandmark(ruleId, role, impact = "moderate") {
    return {
        ruleId,
        run(nodes) {
            const violations = [];
            ...
            if (landmarks.length <= 1) {
                ...
                if (target) {
                    passes.push(target);
                    checkDetails.set(target, {
                        any: [makeCheck(ruleId, impact, "Document has at most one " + role + " landmark")],
                    });
                }
            }
            else {
                ...
                for (const node of landmarks) {
                    ...
                    checkDetails.set(node, {
                        any: [makeCheck(ruleId, impact, "Document has " + landmarks.length + " " + role + " landmarks — should have at most one")],
                    });
                }
            }
            return { violations, passes, checkDetails };
...
/* ------------------------------------------------------------------ */
function topLevelLandmark(ruleId, role) {
    return {
        ruleId,
        run(nodes) {
            const violations = [];
            ...
            for (const node of landmarks) {
                if (isInsideLandmark(node)) {
                    violations.push(node);
                    checkDetails.set(node, {
                        any: [makeCheck(ruleId, "moderate", role + " landmark is nested inside another landmark")],
                    });
                }
                else {
                    passes.push(node);
                    checkDetails.set(node, {
                        any: [makeCheck(ruleId, "moderate", role + " landmark is at the top level")],
                    });
                }
            }
            return { violations, passes, checkDetails };
            ...
            if (landmarks.length <= 1) {
                if (landmarks.length === 1) {
                    ...
                    checkDetails.set(landmarks[0], {
                        any: [makeCheck("landmark-unique", "moderate", "Landmark is unique — only one " + role + " landmark")],
                    });
                }
                continue;
            }
            ...
            for (const [key, nodeList] of nameMap) {
                if (nodeList.length > 1) {
                    for (const node of nodeList) {
                        ...
                        checkDetails.set(node, {
                            any: [makeCheck("landmark-unique", "moderate", "Multiple " + role + " landmarks" +
                    ...
                    checkDetails.set(nodeList[0], {
                        any: [makeCheck("landmark-unique", "moderate", "Landmark has a unique label among " + role + " landmarks")],
                    });
                }
            }
        }
