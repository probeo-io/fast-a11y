...
import { isHiddenOrAncestorHidden, getTextContent } from "../tree.js";
/* ================================================================== */
...
function parseColor(color, varMap) {
    if (!color)
        return null;
...
function resolveVarInString(value, varMap) {
    return value.replace(/var\(\s*(--[\w-]+)\s*(?:,\s*([^)]+))?\s*\)/g, (_, name, fallback) => varMap.get(name.trim()) ?? fallback?.trim() ?? "");
}
/**
 ...
 */
function buildVariableMap(allNodes, extraCss = []) {
...
function getInlineStyleProperty(style, prop) {
...
function buildStyleMap(allNodes, extraCss = []) {
    ...
    for (const css of extraCss)
        styleRules.push(...parseStyleSheet(css));
...
function resolveStyles(node, styleMap, varMap) {
    let color = null;
    ...
    for (const n of chain) {
        ...
        if (rawFs)
            fontSize = (varMap && rawFs.includes("var(")) ? resolveVarInString(rawFs, varMap) : rawFs;
        ...
        if (rawFw)
            fontWeight = (varMap && rawFw.includes("var(")) ? resolveVarInString(rawFw, varMap) : rawFw;
    }
    return { color, backgroundColor, fontSize, fontWeight };
}
/** Try to extract a color from a background shorthand. */
function extractBgColor(bg) {
    if (!bg)
        return null;
...
const colorContrast = {
    ...
    run(nodes, allNodes, context) {
        ...
        const DEFAULT_FG = [0, 0, 0, 1];
        const DEFAULT_BG = [255, 255, 255, 1];
        for (const node of nodes) {
            ...
            const hasDirectText = node.childNodes.some((child) => child.type === "text" && child.data.trim());
            if (!hasDirectText)
                continue;
            ...
            if (hasBackgroundImage) {
                ...
                checkDetails.set(node, {
                    any: [makeCheck("color-contrast", "serious", "Element has a background image — cannot determine contrast ratio")],
                });
                continue;
            }
            ...
            if (level !== "fail") {
                passes.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("color-contrast", "serious", "Element has sufficient contrast ratio of " + ratioStr + ":1 (WCAG " + level + ")" +
                            " (foreground: " + fgStr + ", background: " + bgStr + ")" + sizeLabel, baseData)],
                });
            }
            else {
                ...
                checkDetails.set(node, {
                    any: [makeCheck("color-contrast", "serious", "Element has insufficient contrast ratio of " + ratioStr + ":1" +
                            " (foreground: " + fgStr + ", background: " + bgStr + ")" +
                            ". Required " + required + ":1 for WCAG AA" + sizeLabel, { ...baseData, requiredRatio: required })],
                });
            }
        }
