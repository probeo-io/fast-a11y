...
function getPrimarySubtag(lang) {
    ...
    run(nodes) {
        ...
        for (const node of htmlNodes) {
            ...
            if ((lang && lang.trim()) || (xmlLang && xmlLang.trim())) {
                ...
                checkDetails.set(node, {
                    any: [makeCheck("html-has-lang", "serious", "The <html> element does not have a lang attribute")],
                });
            }
        }
...
const htmlLangValid = {
    ...
    run(nodes) {
        ...
        for (const node of htmlNodes) {
            const lang = node.attrs.lang;
            if (!lang || !lang.trim())
                continue; // html-has-lang handles missing lang
            if (isValidLang(lang)) {
                passes.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("html-lang-valid", "serious", "Value of lang attribute is a valid language: " + lang)],
                });
            }
            else {
                violations.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("html-lang-valid", "serious", "Value of lang attribute is not a valid language: " + lang)],
                });
            }
        }
        return { violations, passes, checkDetails };
...
const htmlXmlLangMismatch = {
    ...
    run(nodes) {
        ...
        for (const node of htmlNodes) {
            ...
            const xmlLang = node.attrs["xml:lang"];
            if (!lang || !xmlLang)
                continue; // only applies when both are present
            ...
            if (langPrimary === xmlLangPrimary) {
                passes.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("html-xml-lang-mismatch", "moderate", "lang and xml:lang attributes have the same primary language")],
                });
            }
            else {
                violations.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("html-xml-lang-mismatch", "moderate", "lang=\"" + lang + "\" and xml:lang=\"" + xmlLang + "\" have different primary languages")],
                });
            }
        }
        return { violations, passes, checkDetails };
...
const validLang = {
    ...
    run(nodes) {
        ...
        for (const node of nodes) {
            if (node.tag === "html")
                continue; // html-lang-valid handles <html>
            ...
            if (isValidLang(lang)) {
                passes.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("valid-lang", "serious", "Value of lang attribute is a valid language: " + lang)],
                });
            }
            else {
                violations.push(node);
                checkDetails.set(node, {
                    any: [makeCheck("valid-lang", "serious", "Value of lang attribute is not a valid language: " + lang)],
                });
            }
        }
        return { violations, passes, checkDetails };
